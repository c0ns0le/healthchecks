# Simply gathers Disk Reads stats in MBps from all VMs for the past 7 Days
...
$IOPSReport = $srvconnection | %{
	...
	$myVMs | %{
		...
		$metaInfo = @()
		$metaInfo +="tableHeader=VMs By Disk Reads (MBps): Last 7 Days"
		$metaInfo +="introduction=Average number of Megabytes read from disk each second during the collection interval."
