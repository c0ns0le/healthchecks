# Simply gathers Disk Latency stats from all VMs for the past 7 Days @  5 minute intervals
...
$IOPSReport = $srvconnection | %{
	...
	$myVMs | %{
		...
		$stats = Get-Stat -Entity $vm -Stat "disk.maxtotallatency.latest" -MaxSamples $numsamples -Start (Get-Date).AddDays(-$numPastDays) -IntervalMins $sampleIntevalsMinutes
		$metaInfo = @()
		$metaInfo +="tableHeader=VMs By Disk Latency (milliseconds): Last 7 Days"
		$metaInfo +="introduction=Highest latency value across all disks used."
