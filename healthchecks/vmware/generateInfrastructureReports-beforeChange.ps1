...
if ($emailReport -or $htmlReports)
{
	...
	if($showThisReport)
	{
		...
		$reportIndexer=1
		logThis -msg "Enumerating through the $($csvList.Count) CSV Files found"
		$color="Cyan"
		#$csvList | %{
		$csvList | %{
			$report = @{}
			...
			Write-Progress -Activity "Processing Reports.." -CurrentOperation "$reportIndexer/$($csvList.Count) :- $($report.Filename)" -PercentComplete ($reportIndexer /  $csvList.Count * 100)
			$report["File"]=$_
			$report["LogFile"] = $_.Replace(".csv",".log")
			$report["NFO"] = $_.Replace(".csv",".nfo")
			#$report.File = $_;
			logThis -msg "++++++++++++++++++++++++++++++++++++++++++"
			logThis -msg "Processing [$($report.Index) of $($csvList.Count)] :- $($report.File)" -foregroundColor $color
			...
			while ($passesSoFar -le $passesToMake) 
			{
				...
				if($report.DataTable)
				{
					...
					if ($report.chartable -eq "true")
					{
						...
						{
							$report.DataTable | %{
								$htmlPage += ($_ | ConvertTo-HTML -Fragment -As $report.displayTableOrientation) -replace "<table","$caption<table class=aITTablesytle"
							}						
						} elseif ($report.displayTableOrientation -eq "Table") {
							#User has specified TABLE
							$htmlPage += ($report.DataTable | ConvertTo-HTML -Fragment -As "Table") -replace "<table","$caption<table class=aITTablesytle"
	...
	if ($emailReport)
	{
		...
		if ($sendAllEmailsTo)
		{
			...
			#-replyTo "$itoContactName <$replyToRecipients>"  `
			.\sendMail-Routine.ps1  -smtpServer $smtpServer `										
									-from "$fromContactName <$fromRecipients>" `
									-replyTo "$itoContactName <$replyToRecipients>" `
			...
			logThis -msg "Emailing report to $toRecipients"
			.\sendMail-Routine.ps1  -smtpServer $smtpServer `
									-from "$fromContactName <$fromRecipients>" `
									-replyTo "$itoContactName <$replyToRecipients>" `
