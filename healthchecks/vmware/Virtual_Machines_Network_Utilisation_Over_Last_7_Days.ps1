# Simply gathers Network stats from all VMs for the past 7 Days @  5 minute intervals
...
$IOPSReport = $srvconnection | %{
	...
	$myVMs | %{
		...
		$stats = Get-Stat -Entity $vm -Stat "net.usage.average" -MaxSamples $numsamples -Start (Get-Date).AddDays(-$numPastDays) -IntervalMins $sampleIntevalsMinutes
		$metaInfo = @()
		$metaInfo +="tableHeader=VMs By Network Utilisation: Last 7 Days"
		$metaInfo +="introduction=Network utilization (combined transmit-rates and receive-rates) in KBps during the interval."
